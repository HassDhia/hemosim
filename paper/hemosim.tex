...
An early and frequently-cited RL-for-anticoagulation paper. Nemati, Ghassemi, and Clifford trained a DQN on a retrospective MIMIC-II heparin cohort and reported a learned dosing policy with improved time-in-therapeutic-range over clinician-administered doses in the held-out cohort. The paper's principled focus was learning from retrospective institutionally-held data; releasing a reusable simulation environment was outside the scope of that work. \texttt{hemosim} targets the reproducibility gap that absence left: the environment is released pip-installable, the Raschke baseline is reimplemented as a shared reference, and the Nemati~2016 DQN architecture is reimplemented (\texttt{src/hemosim/agents/baselines\_extended.py::NematiDQN2016Baseline}) so its performance on a shared environment can be measured alongside other baselines. The calibration section (\S\ref{sec:calibration}) treats a published TTR target for the standard Raschke nomogram as a benchmark (see Table~\ref{tab:heparin_residuals} for the specific numeric target and residual), and our honest residual at the cohort level motivates exactly the POMDP reformulation described in \S\ref{sec:pomdp}.
...
\begin{table}[ht]
\centering
\caption{DOAC event-rate validation (500 episodes per drug, seed~42). Bleed rates for all three drugs fall within the trial 95\% confidence intervals. \emph{All three stroke rates fall outside their trial confidence intervals}: observed rates are above the upper CI bounds for dabigatran (1.43 vs upper bound 1.33), rivaroxaban (3.10 vs upper bound 2.00), and apixaban (1.64 vs upper bound 1.53). This is an honest cohort-level miscalibration of the stroke-event simulator across all three drugs and is flagged for Phase~2 re-calibration against individual-patient MIMIC-IV data; the bleed-event simulator is within CI and does not require the same re-fit.}
