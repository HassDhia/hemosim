...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[
    ...
    \node[policy] (policy) {Policy\\$\pi(a_t \mid o_{\leq t}, a_{<t})$};

    \node[dose,       right=2.2cm of policy] (dose)    {Dose action\\(infusion rate)};
    \node[labaction,  below=0.9cm of dose]   (labact)  {Lab-order action\\(aPTT / anti-Xa / plt)};

    \node[state, right=2.2cm of dose]   (state) {HeparinPKPD\\(true state, hidden)};
    \node[queue, right=2.2cm of labact] (queue) {LabOrderQueue\\(TAT + CV noise)};

    \node[obs, below=1.0cm of queue] (obs) {Observation:\\returned lab history\\$(\ell, v_{\text{noisy}}, t_{\text{ord}}, t_{\text{ret}}, \sigma_\ell)$};

    \draw[arr] (policy.east |- dose.west)   -- (dose.west);
    \draw[arr] (policy.east |- labact.west) -- (labact.west);
    ...
    \draw[arr] (obs.west) -- ++(-2.2,0) |- (policy.south);
